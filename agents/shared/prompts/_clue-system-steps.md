...
You may NOT:
- Edit Python source code in `src/`, `tests/`, `scripts/`
- Modify the production database (INSERT, UPDATE, DELETE)
- Restart or modify systemd services or containers
- Send emails to external recipients
- Edit pipeline prompts in `config/defaults/` or DB prompt fields
- Push git commits or create PRs
- Run destructive bash commands (`rm`, `git reset`, etc.)

If your curiosity requires any of these actions, propose it — do not do it yourself.
